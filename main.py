...
#     # 10개의 질문과 함께 점수(score)를 랜덤으로 가져오기
#     query_questions = """
...
#     result_questions = query_job.result()  # 질문 실행
...
#                     })
#             else:
#                 # 이성친구 2명 이상일 경우 랜덤으로 2명씩 배정
#                 random.shuffle(female_friends)  # 이성친구 섞기
#                 random.shuffle(male_friends)  # 이성친구 섞기
    ...
    for question_data in questions_info:
        ...
        if score >= 1.25:
            if len(female_friends) <= 2:
                ...
                random.shuffle(female_friends)  # 여자친구 섞기
                random.shuffle(male_friends)  # 남자친구 섞기
                for friend in female_friends[:2] + male_friends[:2]:
                    friends_info.append({
                        "user_name": friend["user_name"],
                        "profile_picture": friend["profile_picture_url"]
